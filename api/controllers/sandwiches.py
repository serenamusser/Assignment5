...
def read_all(db: Session):
    # Query the database to retrieve all sandwiches
    return db.query(models.Sandwich).all()

def read_one(db: Session, sandwich_id: int):
    # Query the database for the specific sandwich by its ID
    sandwich = db.query(models.Sandwich).filter(models.Sandwich.id == sandwich_id).first()
    # Raise an HTTP 404 error if the sandwich is not found
    if sandwich is None:
